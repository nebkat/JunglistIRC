...
public class MessageEventParser extends Parser {
    @Override
    public IRCEvent parse(long time, Session session, ParseToken token) {
        ...
        if (channel.equals(session.getNick()) && token.getSource().getNick() != null) {
            channel = token.getSource().getNick();
        }
        Target target = session.getOrInitiateTarget(channel);

        if (token.getCommand().equals(Parser.COMMAND_PRIVMSG)) {
            return new PrivMessageEvent(time, session, token.getRaw(), token.getSource(), target, message);
        } else if (token.getCommand().equals(Parser.COMMAND_NOTICE)) {
            return new NoticeEvent(time, session, token.getRaw(), token.getSource(), target, message);
        } else {
            return null;
        }
    }
